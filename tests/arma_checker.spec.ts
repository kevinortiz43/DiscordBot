...
for (let modIndex = 0; modIndex < randomizedMods.length; modIndex++) {
  ...
  test(`Mod ${name} - Check recent update`, async ({ page }) => {
    // Skip test if rate limit already detected
    if (rateLimitDetected) {
      test.skip();
      return;
    }

    // Add delay between tests (3-5 seconds random)
    ...
    try {
      ...
      if (response?.status() === 429) {
        console.error(`Rate limit detected for mod: ${name}`);
      ...
      if (pageContent.includes("rate limit") || 
        ...
        console.error(`Rate limit detected in page content for mod: ${name}`);
        rateLimitDetected = true;
        test.skip();
        return;
      }
      ...
      if (errorMessage.includes("429") || 
        ...
        console.error(`Rate limit detected via error for mod: ${name}`);
        rateLimitDetected = true;
        test.skip();
        return;
      }
